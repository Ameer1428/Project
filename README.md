...
1. **API Layer** (`api_integration.py`)
   - REST API endpoints for client interactions
   - Request/response processing
   - Core component integration

2. **Workload Allocator** (`workload_allocator.py`)
   - Resource allocation management
   - Energy-aware scheduling
   - AWS EC2 and EKS integration

3. **Energy Monitor** (`energy_monitoring.py`)
   - Real-time energy metrics collection
   - Carbon footprint tracking
   - Metrics publishing

4. **ML Predictor** (`ml_training.py`)
   - Workload prediction models
   - Demand forecasting
   - Resource optimization

5. **Data Storage**
   - AWS S3: Metrics storage
   - Local: ML models
   - CloudWatch: Time-series data
...
7. **Cloud Resources**
   - AWS EC2: VM workloads
   - AWS EKS: Container workloads
